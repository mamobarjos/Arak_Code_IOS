...
class HomeViewController: UIViewController {
    ...
    private func fetchAds() {
        if screenType == .Favrate {
            ...
            showLoading()
            viewModel.adsList(page: pageAds, search: "") { [weak self] (error) in
...
extension HomeViewController: UICollectionViewDelegate , UICollectionViewDataSource , UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if (viewModel.itemBannerCount > 0 && screenType == .Home) && viewModel.itemFeaturedCount > 0  {
            ...
            let vc = initViewControllerWith(identifier: StoryViewController.className, and: "") as! StoryViewController
            vc.config(viewModel: StoryViewModel(adsList: viewModel.getAll(), index: indexPath.row - 2), source: screenType)
            show(vc)
        } else if viewModel.itemFeaturedCount > 0  {
            if indexPath.row == 0 {
                let vc = initViewControllerWith(identifier: StoryViewController.className, and: "") as! StoryViewController
            ...
            let vc = initViewControllerWith(identifier: StoryViewController.className, and: "") as! StoryViewController
            vc.config(viewModel: StoryViewModel(adsList: viewModel.getAll(), index: indexPath.row - 1), source: screenType)
            show(vc)
        } else {
            let vc = initViewControllerWith(identifier: StoryViewController.className, and: "") as! StoryViewController
